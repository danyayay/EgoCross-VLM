"""Qwen2.5-VL adapter (Qwen/Qwen2.5-VL-7B-Instruct).

Qwen2.5-VL uses Qwen2_5_VLForConditionalGeneration (distinct from Qwen3VL).
The inference interface is otherwise identical to the Qwen3VL adapter.
"""
...
class Qwen25VLAdapter(VLMAdapter):
    ...
    def get_peft_target_modules(self, ft_type: str) -> list[str]:
        target_modules_by_type = {
            ...
            'lora_vlm_bridger':       ['merger.mlp.0', 'merger.mlp.2'],
        }
        if ft_type not in target_modules_by_type:
            raise ValueError(f"Unsupported ft_type {ft_type} for Qwen25VLAdapter")
