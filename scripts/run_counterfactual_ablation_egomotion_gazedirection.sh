...
LOG_DIR_ROOT="${LOG_DIR_ROOT:-logs/counterfactual_ablation_egomotion_gaze_direction}"
...
CONTEXT_FEATURE_SETS="${CONTEXT_FEATURE_SETS:-ego_motion,gaze_direction}"
...
for adapter_path in $ADAPTER_PATHS; do
    ...
    common=(
        ...
        --context_feature_fps "$CONTEXT_FEATURE_FPS_SET"
        --context_prompt_mode "$CONTEXT_PROMPT_MODES"
        --context_feature_format "$CONTEXT_FEATURE_FORMATS"
        --context_feature_interpretation "$CONTEXT_FEATURE_INTERPRETATIONS"
...
curl -d "✅ Done!" \
    -H "Counterfactual analysis, ego motion and gaze direction evaluation" \
    -H "Priority: high" \
